...
public sealed class AMEControllerComponent : SharedAMEControllerComponent
{
    ...
    [ViewVariables] private BoundUserInterface? UserInterface => _userInterface.GetUiOrNull(Owner, AMEControllerUiKey.Key);
    ...
    protected override void Initialize()
    {
        ...
        JarSlot = containerSystem.EnsureContainer<ContainerSlot>(Owner, $"AMEController-fuelJarContainer");
    }
    ...
    public void UpdateUserInterface()
    {
        ...
        if (UserInterface != null)
            _userInterface.SetUiState(UserInterface, state);
    }
    ...
    private void ToggleInjection()
    {
        if (!_injecting)
        {
            _appearanceSystem.SetData(Owner, AMEControllerVisuals.DisplayState, "on");
            //_appearance?.SetData(AMEControllerVisuals.DisplayState, "on");
        }
        else
        {
            _appearanceSystem.SetData(Owner, AMEControllerVisuals.DisplayState, "off");
    ...
    private void UpdateDisplay(int stability)
    {
        ...
        _appearanceSystem.TryGetData<string>(Owner, AMEControllerVisuals.DisplayState, out var state);
        ...
        if (state != newState)
        {
            _appearanceSystem.SetData(Owner, AMEControllerVisuals.DisplayState, newState);
    ...
    private void ClickSound()
    {
        ...
        sharedAudioSystem.Play(_clickSound, Filter.Pvs(Owner), Owner, false, AudioParams.Default.WithVolume(-2f));
    }

    private void InjectSound(bool overloading)
    {
        var sharedAudioSystem = IoCManager.Resolve<SharedAudioSystem>();
        sharedAudioSystem.Play(_injectSound, Filter.Pvs(Owner), Owner, false, AudioParams.Default.WithVolume(overloading ? 10f : 0f));
    }
}
