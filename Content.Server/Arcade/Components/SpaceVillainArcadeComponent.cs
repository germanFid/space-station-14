...
public sealed class SpaceVillainArcadeComponent : SharedSpaceVillainArcadeComponent
{
    ...
    [ViewVariables] private BoundUserInterface? UserInterface => _userInterfaceSystem.GetUiOrNull(Owner, SpaceVillainArcadeUiKey.Key);
    ...
    private List<string> _possibleFightVerbs = new()
    ...
    private List<string> _possibleFirstEnemyNames = new(){
    /// Corvax-Localization-Start
    ...
    private List<string> _possibleLastEnemyNames = new()
    {
    /// Corvax-Localization-Start
    ...
    private void UserInterfaceOnOnReceiveMessage(ServerBoundUserInterfaceMessage serverMsg)
    {
        ...
        switch (msg.PlayerAction)
        {
            ...
            case PlayerAction.NewGame:
                _sharedAudioSystem.Play(_newGameSound, Filter.Pvs(Owner), Owner, false, AudioParams.Default.WithVolume(-4f));

                Game = new SpaceVillainGame(this);
                if (UserInterface != null)
                    _userInterfaceSystem.SendUiMessage(UserInterface, Game.GenerateMetaDataMessage());
                break;
    ...
    public sealed class SpaceVillainGame
    {
        ...
        public void ExecutePlayerAction(PlayerAction action)
        {
            ...
            switch (action)
            {
                case PlayerAction.Attack:
                    ...
                    _sharedAudioSystem.Play(_owner._playerAttackSound, Filter.Pvs(_owner.Owner), _owner.Owner, false, AudioParams.Default.WithVolume(-4f));
                ...
                case PlayerAction.Heal:
                    ...
                    _sharedAudioSystem.Play(_owner._playerHealSound, Filter.Pvs(_owner.Owner), _owner.Owner, false, AudioParams.Default.WithVolume(-4f));
                ...
                case PlayerAction.Recharge:
                    ...
                    _sharedAudioSystem.Play(_owner._playerChargeSound, Filter.Pvs(_owner.Owner), _owner.Owner, false, AudioParams.Default.WithVolume(-4f));
        ...
        private bool CheckGameConditions()
        {
            if ((_playerHp > 0 && _playerMp > 0) && (_enemyHp <= 0 || _enemyMp <= 0))
            {
                ...
                UpdateUi(Loc.GetString("space-villain-game-player-wins-message"),
                         ...
                         true);
                _sharedAudioSystem.Play(_owner._winSound, Filter.Pvs(_owner.Owner), _owner.Owner, false, AudioParams.Default.WithVolume(-4f));
                _owner.ProcessWin();
                return false;
            }
            ...
            if ((_enemyHp > 0 && _enemyMp > 0))
            {
                ...
                UpdateUi(Loc.GetString("space-villain-game-player-loses-message"),
                         ...
                         true);
                _sharedAudioSystem.Play(_owner._gameOverSound, Filter.Pvs(_owner.Owner), _owner.Owner, false, AudioParams.Default.WithVolume(-4f));
                return false;
            }
            if (_enemyHp <= 0 || _enemyMp <= 0)
            {
        ...
        {
            if (_owner.UserInterface != null)
                _userInterfaceSystem.SendUiMessage(_owner.UserInterface, metadata ? GenerateMetaDataMessage() : GenerateUpdateMessage());
        }
