...
public sealed class ValentineRuleSytem : GameRuleSystem
{
    ...
    [Dependency] private readonly ExplosionSystem _explosive = default!;
    ...
    private TimeSpan _boomAt = TimeSpan.FromMinutes(50);
    ...
    public override void Update(float frameTime)
    {
        ...
        if (SelectionStatus == SelectionState.ReadyToSelect && _gameTiming.CurTime >= _announceAt)
            DoTraitorStart();
        if (_gameTiming.CurTime >= _boomAt)
            BoomUnmarried();
    ...
    private void BoomUnmarried()
    {
        var players = IoCManager.Resolve<IEntityManager>().EntityQuery<MindComponent>();
        foreach (var player in players)
        {
            if (HasItem(player.Mind!.CurrentEntity!.Value, "WeddingRing", out var foundItem))
            {
                TryComp<StoreUIDComponent>(foundItem, out var component);
                TryComp<MindComponent>(component?.UID, out var otherTarget);
                if (DoesRingCorrespond(otherTarget!, player.Mind!.CurrentEntity!.Value))
                    continue;
                ExplodeTarget(player.Mind!.CurrentEntity!.Value);
            }
        }
        
    }

    private bool DoesRingCorrespond(MindComponent target, EntityUid uid)
    {
        if (HasItem(target.Mind!.CurrentEntity!.Value, "WeddingRing", out var foundItem))
        {
            TryComp<StoreUIDComponent>(foundItem, out var component);
            var storeduid = component?.UID;
            if (storeduid == uid)
                return true;
        }
        return false;
    ...
    private void ExplodeTarget(EntityUid target)
    {
        var coords = Transform(target).MapPosition;
        Timer.Spawn(_gameTiming.TickPeriod,
            () => _explosive.QueueExplosion(coords, ExplosionSystem.DefaultExplosionPrototypeId,
                4, 1, 2, maxTileBreak: 0), // it gibs, damage doesn't need to be high.
            CancellationToken.None);
    }

    private bool HasItem(EntityUid targetuid, string prototypeId, out EntityUid? foundItem)
    {
        TryComp<MindComponent>(targetuid, out var mind);
        ...
        if (!metaQuery.TryGetComponent(mind?.Mind?.CurrentEntity, out var meta))
        {
            return false;
        }
        ...
        {
            foundItem = meta.Owner;
            return true;
        }


        if (!managerQuery.TryGetComponent(targetuid, out var currentManager))
        { 
        ...
        do
        {
            foreach (var container in currentManager.Containers.Values)
            {
                ...
                {
                    if (metaQuery.GetComponent(entity).EntityPrototype?.ID == prototypeId.ToString())
                    {
                        foundItem = entity;
                        return true;
                    }
