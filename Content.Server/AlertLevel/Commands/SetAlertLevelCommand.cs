...
public sealed class SetAlertLevelCommand : IConsoleCommand
{
    ...
    public CompletionResult GetCompletion(IConsoleShell shell, string[] args)
    {
        ...
        if (player?.AttachedEntity != null)
        {
            ...
            var stationUid = entityManager.System<StationSystem>().GetOwningStation(player.AttachedEntity.Value);
            if (stationUid != null)
            {
    ...
    public void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        entityManager.System<AlertLevelSystem>().SetLevel(stationUid.Value, level, true, true, true, locked);
    }

    private static string[] GetStationLevelNames(EntityUid station)
    {
        var entityManager = IoCManager.Resolve<IEntityManager>();
