...
public sealed class AirlockPainterSystem : SharedAirlockPainterSystem
{
    ...
    private void OnActivate(EntityUid uid, AirlockPainterComponent component, ActivateInWorldEvent args)
    {
        ...
        var boundUi = _userInterfaceSystem.GetUiOrNull(component.Owner, AirlockPainterUiKey.Key);
        if (boundUi == null)
            throw new Exception($"Unexpected null reference. {nameof(boundUi)} was null.");
        _userInterfaceSystem.OpenUi(boundUi, actor.PlayerSession);
    ...
    private void AfterInteractOn(EntityUid uid, AirlockPainterComponent component, AfterInteractEvent args)
    {
        ...
        {
            var msg = Loc.GetString("airlock-painter-style-not-available");
